...
def export_clinical_report(user_id: int, days: int = 30, 
    ...
    try:
        ...
        # Assessment comparison (may return None)
        try:
            comparison = psychometrics.compare_pre_post_assessment(db, user_id)
        except Exception:
            comparison = None
        ...
        # Build patient info with field names the PDF expects
        user_info = dashboard.get("user_info", {})
        patient = {
            "user_id": user_info.get("id", user.id),
            "username": user_info.get("username", user.username),
            "age_group": user_info.get("age_group", user.age_group),
            "hearing_level": user_info.get("hearing_level", user.hearing_level),
            "current_level": user_info.get("current_level", user.current_level),
            "total_score": user.total_score,
            "created_at": user.created_at.isoformat() if user.created_at else None,
        }
        
        # Build training_summary with per-scenario breakdown and reaction times
        dash_summary = dashboard.get("training_summary", {})
        progress_data = progress if isinstance(progress, dict) and "error" not in progress else {}
        
        # Compute per-scenario data from attempts
        cutoff_date = datetime.utcnow() - timedelta(days=days)
        period_attempts = db.query(models.Attempt).filter(
            models.Attempt.user_id == user_id,
            models.Attempt.timestamp >= cutoff_date
        ).all()
        ...
        for a in period_attempts:
            ...
            if sc not in per_scenario:
                per_scenario[sc] = {"attempts": 0, "successes": 0, "rts": []}
            per_scenario[sc]["attempts"] += 1
            if a.success:
                per_scenario[sc]["successes"] += 1
            if a.reaction_time and a.reaction_time > 0:
                per_scenario[sc]["rts"].append(a.reaction_time)
                all_rts.append(a.reaction_time)
        
        per_scenario_clean = {}
        for sc, d in per_scenario.items():
            per_scenario_clean[sc] = {
                "attempts": d["attempts"],
                "successes": d["successes"],
                "avg_rt": round(float(np.mean(d["rts"])), 2) if d["rts"] else None,
            }
        
        total_attempts = dash_summary.get("total_attempts", 0)
        total_sessions = dash_summary.get("total_sessions", 0)
        overall_accuracy = dash_summary.get("overall_accuracy", 0)
        
        training_summary = {
            "total_sessions": total_sessions,
            "total_attempts": total_attempts,
            "success_rate": round(overall_accuracy / 100, 3) if overall_accuracy else 0,
            "avg_reaction_time": round(float(np.mean(all_rts)), 2) if all_rts else None,
            "per_scenario": per_scenario_clean if per_scenario_clean else None,
        }
        
        # Convert clinical_recommendations from dicts to plain strings for PDF
        raw_recs = dashboard.get("clinical_recommendations", [])
        clinical_recs = []
        for r in raw_recs:
            if isinstance(r, dict):
                clinical_recs.append(r.get("suggestion", r.get("area", str(r))))
            else:
                clinical_recs.append(str(r))
        ...
        return {
            ...
            "progress_over_period": progress_data if progress_data else None,
            "pre_post_comparison": comparison,
        ...
        import traceback
        print(f"❌ Clinical report export error for user {user_id}: {e}")
        traceback.print_exc()
